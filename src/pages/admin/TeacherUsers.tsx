...
const TeacherUsers = () => {
  ...
  const fetchUsers = async (pageNum: number) => {
    try {
      ...
      setError(err?.response?.data?.error || "Error al cargar los profesores");
  ...
  useEffect(() => {
    if (location.state && (location.state as any).openCreateDrawer) {
  ...
  const handleEdit = (user: TeacherUser) => {
  ...
  const handleObservations = (user: TeacherUser) => {
    const fullName =
  ...
  const handleUserCreated = (newUser: TeacherUser) => {
      ...
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto flex items-start sm:items-center gap-2">
          ...
          <h1 className="text-sm  text-gray-700">
            Profesores con acceso al sistema de gestión académica.
          </h1>
        </div>
