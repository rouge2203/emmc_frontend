...
const SET_PASSWORD_URL = "auth/set-password";
...
/**
 * Optional step after activating an account via email login. A password is
 * genuinely optional here — email login remains available — so skipping
 * must be just as easy as submitting.
 */
const SetPassword = () => {
  ...
  const destination = getRoleLadderPath(auth?.user?.role);
  ...
  }, [isLoading, passwordRequirements]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      await axiosPrivate.post(SET_PASSWORD_URL, { password });
      ...
      if (!error?.response) {
        ...
        setErrMsg(
          error.response?.data?.error || "No se pudo crear la contraseña",
        );
      } else {
        setErrMsg("No se pudo crear la contraseña");
      }
      errRef.current?.focus();
    } finally {
      setIsLoading(false);
            ...
            <div className="lg:w-1/2 p-8 lg:p-12 flex flex-col justify-center overflow-y-auto">
              <div className="w-full max-w-md mx-auto h-full">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-2 text-center">
                  Crea una contraseña (opcional)
                </h2>
                <p className="text-sm text-gray-600 mb-6 text-center">
                  Ya iniciaste sesión con el código que te enviamos por
                  correo. Si quieres, puedes crear una contraseña para
                  entrar más rápido la próxima vez — siempre podrás seguir
                  iniciando sesión con un código.
                </p>
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                  ...
                  >
                    {isLoading ? "Guardando contraseña..." : "Crear contraseña"}
                  </button>
                  ...
                  <button
                    ...
                    onClick={handleSkip}
                    disabled={isLoading}
                    className="w-full border-2 border-gray-900 text-gray-900 font-semibold py-3 px-4 rounded-lg hover:bg-gray-50 transition-colors disabled:border-gray-300 disabled:text-gray-400 disabled:hover:bg-white flex items-center justify-center"
                  >
                    Omitir por ahora
                  </button>
                </form>
                ...
                <div className="mt-3 text-center sm:mt-5">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Ya puedes iniciar sesión con tu contraseña, o seguir
                      usando el código que te enviamos por correo.
                    </p>
                  </div>
                </div>
              </div>
